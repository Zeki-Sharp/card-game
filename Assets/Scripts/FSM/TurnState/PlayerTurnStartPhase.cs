...
namespace ChessGame.FSM.TurnState
{
    ...
    /// 玩家回合开始阶段
    /// </summary>
    public class PlayerTurnStartPhase : TurnStateBase
    {
        private Coroutine _phaseCoroutine;
        
        public PlayerTurnStartPhase(TurnStateMachine stateMachine) : base(stateMachine) { }
        
        public override void Enter()
        {
            Debug.Log("进入玩家回合开始阶段");
            
            // 通知游戏事件系统
            GameEventSystem.Instance.NotifyTurnStarted(0);
            
            // 重置所有玩家卡牌的行动状态
            StateMachine.GetCardManager().ResetPlayerCardActions();
        ...
        {
            Debug.Log("触发玩家回合开始能力");
            
            // 这里可以添加触发回合开始能力的代码
            // 例如遍历所有玩家卡牌，检查是否有回合开始触发的能力
        }
        ...
        private IEnumerator DelayedCompletePhase()
        {
            ...
            // 完成当前阶段，进入主要阶段
            CompletePhase(TurnPhase.PlayerMainPhase);
        }
    }
} 